...
export class AuthRepository extends Repository<User> {
    ...
    async signin(signinDto: SigninDto): Promise<string> {
        ...
        if (user_with_id || user_with_email) {
            if (user_with_id && user_with_id.password === password) { return user_with_id.toString(); }
            if (user_with_email && user_with_email.password === password) { return user_with_email.toString(); }
        } else {
            return null;
        }
    }
